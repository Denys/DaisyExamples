/*
 *  Simple Granular Synth: USB input version
 *  Brent Edstrom, 2020
 *  
 *  Note: Set board to type Audio in Tools...USB Type menu
 */
...
AudioInputUSB            usb1;           //xy=102,124
AudioMixer4              mixer;          //xy=336,131
AudioEffectGranular      granular1;      //xy=500,131
AudioOutputI2S           i2s1;           //xy=737,141
AudioConnection          patchCord1(usb1, 0, mixer, 0);
AudioConnection          patchCord2(usb1, 1, mixer, 1);
AudioConnection          patchCord3(mixer, granular1);
...
//TO DO: Adapt for code audio input, not USB audio input
...
//Bounce soundBtn = Bounce(8, 15);
//Bounce btn4 = Bounce(16, 15);

//Set up encoders
Encoder lengthEncoder(0, 2);
Encoder speedEncoder(3, 5);
//Encoder encoder2(7, 12);
//Encoder encoder4(14, 17);
...
void loop() 
{
    float half_vol = usb1.volume() / 2.0;
    mixer.gain(0, half_vol);
    mixer.gain(1, half_vol);
...
void handlePitchShiftBtn()
{
    ...
    if (beginPitchShiftBtn.fallingEdge()) 
    {
        if(!speeding)
        {
            ...
            if(temp_grain_length > GRANULAR_MEMORY_SIZE /3)
            {
                 temp_grain_length = GRANULAR_MEMORY_SIZE / 3;
            }
...
void handleLengthEncoder()
{
    ...
    //Uncomment to re-start freezing and shifting as length changes
    if(freezing)
    {
        granular1.beginFreeze(grainLength);
    }
