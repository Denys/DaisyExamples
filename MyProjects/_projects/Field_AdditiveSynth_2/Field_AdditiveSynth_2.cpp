// Field_AdditiveSynth_2.cpp
// 8-voice polyphonic comparison synthesizer for Daisy Field
// OscillatorBank + Adsr per voice
// Global: LFO (tutti vibrato/tremolo), Chorus, ReverbSc
// SW1/SW2 dual knob pages with pickup/catch
// A1-A8: comparison presets | B1-B8: LFO waveform + performance toggles
...
static constexpr int   NUM_REGISTRATIONS = 7;
static constexpr float NORM_TARGET       = 0.9f;   // OscillatorBank expects sum <= 1
...
// ── Registration amplitude buffer (shared, normalized) ──
float registration_amps[NUM_REGISTRATIONS];
...
void RecalcRegistrations() {
    float octave_1 = (rolloff > 0.001f) ? powf(1.f / 1.f, rolloff) : 1.0f;
    float octave_2 = (rolloff > 0.001f) ? powf(1.f / 2.f, rolloff) : 1.0f;
    float octave_4 = (rolloff > 0.001f) ? powf(1.f / 4.f, rolloff) : 1.0f;
    float octave_8 = (rolloff > 0.001f) ? powf(1.f / 8.f, rolloff) : 1.0f;

    registration_amps[0] = octave_1 * (0.20f + 0.90f * even_level);
    registration_amps[1] = octave_1 * (0.25f + 0.60f * odd_level);
    registration_amps[2] = octave_2 * (0.95f * even_level);
    registration_amps[3] = octave_2 * (0.75f * odd_level);
    registration_amps[4] = octave_4 * (0.90f * even_level);
    registration_amps[5] = octave_4 * (0.65f * odd_level);
    registration_amps[6] = octave_8 * (0.70f * even_level + 0.20f * odd_level);

    float sum = 0.f;
    for(int i = 0; i < NUM_REGISTRATIONS; ++i)
        sum += registration_amps[i];

    if(sum > 0.001f) {
        float scale = NORM_TARGET / sum;
        for(int i = 0; i < NUM_REGISTRATIONS; ++i)
            registration_amps[i] *= scale;
    }

    voice_mgr.SetAmplitudes(registration_amps);
}
...
void DrawNormalView() {
    ...
    // Row 1 (y=16): 7 registration bars spanning the OLED width
    float maxAmp = 0.001f;
    for(int i = 0; i < NUM_REGISTRATIONS; ++i)
        if(registration_amps[i] > maxAmp) maxAmp = registration_amps[i];

    for(int i = 0; i < NUM_REGISTRATIONS; ++i) {
        int barH = static_cast<int>((registration_amps[i] / maxAmp) * 30.f);
        int x = i * 18;
        hw.display.DrawRect(x, 46 - barH, x + 14, 46, true, true);
    }
...
void ProcessKeys() {
    // A row: comparison presets (keys 0-7)
    for(int i = 0; i < 8; i++) {
...
void HandleMidiMessage(MidiEvent m) {
    if(m.type == NoteOn) {
        ...
        if(p.velocity == 0) {
            ...
            voice_mgr.NoteOn(p.note, p.velocity, registration_amps);
        }
...
int main(void) {
    ...
    // Compute initial bank registrations and apply ADSR to all voices
    RecalcRegistrations();
    ...
    hw.display.Fill(false);
    hw.display.SetCursor(0, 0);
    hw.display.WriteString("AddSynth_2", Font_7x10, true);
    hw.display.SetCursor(0, 16);
    hw.display.WriteString("8-voice OscBank", Font_6x8, true);
    hw.display.Update();
