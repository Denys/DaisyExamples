...
bool  TouchMoved(float raw_value, float anchor);
...
int PercentInt(float value)
{
    return static_cast<int>(Clamp01(value) * 100.0f + 0.5f);
}
...
void SerialReceiveCallback(uint8_t* buffer, uint32_t* size)
{
    ...
    for(uint32_t i = 0; i < *size; ++i)
    {
        ...
        if(serial_line_index < kSerialLineBufferSize - 1)
        {
            serial_line_buffer[serial_line_index] = ch;
            ++serial_line_index;
        }
        else
        {
...
void LogBoot(const char* stage)
{
    hw.seed.PrintLine("[FTJUNE] BOOT stage=%s ms=%lu",
                      stage,
...
void LogSnapshot(const char* reason)
{
    ...
    hw.seed.PrintLine("[FTJUNE] SNAP_MAIN p=%d,%d,%d,%d,%d,%d,%d,%d cap=%02X",
                      ...
                      PercentInt(MainValue(2)),
                      PercentInt(MainValue(3)),
                      PercentInt(MainValue(4)),
                      PercentInt(MainValue(5)),
                      PercentInt(MainValue(6)),
                      PercentInt(MainValue(7)),
                      CapturedMask(ParamBank::Main));
    hw.seed.PrintLine("[FTJUNE] SNAP_ALT p=%d,%d,%d,%d,%d,%d,%d,%d cap=%02X",
                      PercentInt(AltValue(0)),
                      PercentInt(AltValue(1)),
                      PercentInt(AltValue(2)),
                      PercentInt(AltValue(3)),
                      PercentInt(AltValue(4)),
                      PercentInt(AltValue(5)),
                      PercentInt(AltValue(6)),
                      PercentInt(AltValue(7)),
                      CapturedMask(ParamBank::Alt));
}

bool RunSoftwareSelfTest(const char* reason)
{
    const bool check_defaults = CommandEquals(reason, "boot");
    bool       ok             = true;
    ok                        = ok && !TouchMoved(0.50f, 0.50f);
    ok                        = ok && TouchMoved(0.514f, 0.50f);
    ...
    if(check_defaults)
    {
        for(int i = 0; i < kTemplateKnobCount; ++i)
        {
            ok = ok && fabsf(MainValue(i) - kMainDefaults[i]) < 0.0001f;
            ok = ok && fabsf(AltValue(i) - kAltDefaults[i]) < 0.0001f;
        }
        ok = ok && fabsf(state.output_level - kOutputLevelDefault) < 0.0001f;
    }

    telemetry.selftest_passed = ok;
    if(ok)
        ++telemetry.selftest_pass_count;
    else
...
void ProcessSerialCommands()
{
    ...
    char command[kSerialLineBufferSize] = {};
    for(int i = 0; i < kSerialLineBufferSize; ++i)
    {
        command[i] = serial_line_buffer[i];
        if(command[i] == '\0')
            break;
    }
    ...
    ++telemetry.serial_command_count;

    if(CommandEquals(command, "HELP"))
    {
        hw.seed.PrintLine("[FTJUNE] COMMANDS HELP STATUS SNAP SELFTEST");
    }
    else if(CommandEquals(command, "STATUS"))
    {
        ++telemetry.seq;
        LogStatus("command");
    }
    else if(CommandEquals(command, "SNAP"))
    {
        LogSnapshot("command");
    }
    else if(CommandEquals(command, "SELFTEST"))
    {
        RunSoftwareSelfTest("command");
    }
    else
    {
...
void EmitTelemetryIfDue()
{
    ...
    if(now - telemetry.last_status_ms < kTelemetryPeriodMs)
        return;

    telemetry.last_status_ms = now;
    ++telemetry.seq;
    LogStatus("periodic");
}
...
void RecordTouchAnchors(ParamBank bank, const float raw_knobs[8])
{
    ...
    for(int i = 0; i < kTemplateKnobCount; ++i)
        anchors[i] = Clamp01(raw_knobs[i]);
}

void InitializeTouchAnchorsIfNeeded(const float raw_knobs[8])
{
    if(state.touch_anchors_initialized)
        return;
...
bool TouchMoved(float raw_value, float anchor)
{
    return fabsf(Clamp01(raw_value) - Clamp01(anchor)) > kTouchThreshold;
}
...
void SetActiveBank(ParamBank bank, const float raw_knobs[8])
{
    InitializeTouchAnchorsIfNeeded(raw_knobs);
    if(state.active_bank == bank)
        return;

    state.active_bank = bank;
    state.banks.SetActiveBank(bank);
    RecordTouchAnchors(bank, raw_knobs);
    MarkDisplayDirty();
    MarkLedsDirty();
}
...
void ProcessOutputLevel(const float raw_knob)
{
    ...
    {
        if(!TouchMoved(raw_knob, state.output_touch_anchor))
            return;

        state.output_captured = true;
        ++telemetry.output_capture_count;
        MarkLedsDirty();
    }

    if(fabsf(raw_knob - state.output_level) > kParamChangeDeadband)
    {
        state.output_level = Clamp01(raw_knob);
        ++telemetry.output_write_count;
        char value_text[24];
...
void ProcessBankKnobs(ParamBank bank, const float raw_knobs[8], bool override_level)
{
    ...
    for(int i = 0; i < kTemplateKnobCount; ++i)
    {
        ...
        if(!state.banks.IsCaptured(bank, i))
        {
            if(!TouchMoved(raw_knobs[i], anchors[i]))
                continue;

            state.banks.SetCaptured(bank, i, true);
            ++telemetry.param_capture_count;
            MarkLedsDirty();
        }

        const float current = state.banks.Read(bank, i);
        const float updated = Clamp01(raw_knobs[i]);
        if(fabsf(updated - current) > kParamChangeDeadband)
        {
            state.banks.Write(bank, i, updated);
            ++telemetry.param_write_count;
...
void UpdateUiOutputs(bool sw2_pressed)
{
    const uint32_t now = System::GetNow();

    if(state.led_dirty || (now - state.last_led_update_ms) >= kLedUpdateMs)
    {
...
void ShowStartupDisplay()
{
    hw.display.Fill(false);
    hw.display.SetCursor(0, 0);
    hw.display.WriteString("Field Template June", Font_7x10, true);
    hw.display.SetCursor(0, 16);
    hw.display.WriteString("Until touched controls", Font_6x8, true);
    hw.display.SetCursor(0, 28);
    hw.display.WriteString("Priming ADC/UI state", Font_6x8, true);
    hw.display.Update();
}
...
int main(void)
{
    ...
    telemetry.last_status_ms = NowMs() - kTelemetryPeriodMs;
    LogBoot("audio_started");
