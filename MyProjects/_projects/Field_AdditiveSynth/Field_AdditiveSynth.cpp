...
static const int knob_leds[8]  = {DaisyField::LED_KNOB_1,
...
static const int key_a_leds[8] = {DaisyField::LED_KEY_A1,
...
static const int key_b_leds[8] = {DaisyField::LED_KEY_B1,
...
struct AdditiveVoice
{
    ...
    void SetAmplitudes(const float* amps) { osc.SetAmplitudes(amps); }
...
void ApplyPageA(int k, float v)
{
    switch(k)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            atk_s = LogTimeS(v, 2000.f);
            voice_mgr.SetEnvAll(atk_s, dec_s, sus_lv, rel_s);
            break;
        case 4:
            dec_s = LogTimeS(v, 2000.f);
            voice_mgr.SetEnvAll(atk_s, dec_s, sus_lv, rel_s);
            break;
        case 5:
            sus_lv = v;
...
void DrawNormalView()
{
    ...
    float maxAmp = 0.001f;
    for(int i = 0; i < NUM_PARTIALS; i++)
        if(partial_amps[i] > maxAmp)
            maxAmp = partial_amps[i];
    ...
    if(reverb_bypass)
        strncat(bot, "[R]", sizeof(bot) - strlen(bot) - 1);
    if(chorus_bypass)
        strncat(bot, "[C]", sizeof(bot) - strlen(bot) - 1);
...
void DrawZoomView()
{
    ...
    static const char* pageA_names[8] = {"Rolloff",
                                         "Even",
                                         "Odd",
                                         "Attack",
                                         "Decay",
                                         ...
                                         "Release",
                                         "Rev Mix"};
    static const char* pageB_names[8] = {"LFO Rate",
                                         "LFO Dep",
                                         "LFO Tgt",
                                         "Ch.Rate",
                                         "Ch.Dep",
    ...
    char title[32];
    snprintf(
        title, sizeof(title), "%s%s", locked ? "(L) " : "", names[zoom_param]);
    hw.display.SetCursor(0, 0);
    ...
    if(active_page == 0)
    {
        switch(zoom_param)
        {
            ...
            case 3:
                snprintf(
                    val, sizeof(val), "%.0f ms", LogTimeS(v, 2000.f) * 1000.f);
                break;
            case 4:
                snprintf(
                    val, sizeof(val), "%.0f ms", LogTimeS(v, 2000.f) * 1000.f);
            ...
            case 6:
                snprintf(
                    val, sizeof(val), "%.0f ms", LogTimeS(v, 4000.f) * 1000.f);
                break;
    ...
    else
    {
        ...
        {
            case 0:
                snprintf(val, sizeof(val), "%.2f Hz", LogHz(v, 0.1f, 20.f));
                break;
            case 1:
            {
                float semis = v * 2.0f;
                snprintf(
                    val, sizeof(val), "%d%% +/-%.1fst", (int)(v * 100), semis);
                break;
            }
            case 2:
                snprintf(val, sizeof(val), "%s", v < 0.5f ? "Pitch" : "Amp");
                break;
            case 3:
                snprintf(val, sizeof(val), "%.2f Hz", LogHz(v, 0.1f, 5.f));
                break;
...
void ProcessKeys()
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        {
            lfo_wave                  = i;
            static const int waves[4] = {Oscillator::WAVE_SIN,
                                         Oscillator::WAVE_TRI,
                                         Oscillator::WAVE_SAW,
    ...
    if(hw.KeyboardRisingEdge(13))
        reverb_bypass = !reverb_bypass;
    // B7: Chorus bypass toggle
    if(hw.KeyboardRisingEdge(14))
        chorus_bypass = !chorus_bypass;
    // B8: Sustain toggle
    if(hw.KeyboardRisingEdge(15))
        sustain = !sustain;
                   ...
                   size_t                    size)
{
    ...
    for(size_t i = 0; i < size; i++)
    {
        ...
        if(pitch_scale != 1.0f)
        {
            ...
            {
                if(v.active)
                    v.osc.SetFreq(mtof(v.note) * pitch_scale);
            }
        }
