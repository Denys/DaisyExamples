...
namespace
{
    ...
    float DSY_SDRAM_BSS delayStorage[DaisyDelayFxCore::kDelayLineCount]
    ...
    const char* CurrentAlgorithmLabel()
    {
        ...
        return daisyhost::GetDaisyDelayFxAlgorithmDescriptor(
                   activeCore->GetSource())
            .label;
    }
    ...
    {
        return hw.KeyboardRisingEdge(
            KeyboardIndexForPhysicalKey(row, oneBasedKey));
    }

    bool PhysicalKeyFallingEdge(FieldKeyRow row, size_t oneBasedKey)
    {
        return hw.KeyboardFallingEdge(
            KeyboardIndexForPhysicalKey(row, oneBasedKey));
    }
                    ...
                    int                          knob)
    {
        ...
        std::snprintf(
            zoomName, sizeof(zoomName), "%s", parameter->label.c_str());
    ...
    void RecordAlgorithmZoom()
    {
        ...
        std::snprintf(
            zoomValue, sizeof(zoomValue), "%.23s", CurrentAlgorithmLabel());
    ...
    void SelectBundleAlgorithm(std::size_t index)
    {
        ...
        SaveBundleSnapshot();
        activeCore->SetSource(
            daisyhost::DaisyDelayFxSourceForAlgorithmIndex(index));
    ...
    void ProcessLayeredKnobs()
    {
        ...
        for(std::size_t i = 0; i < knobValues.size(); ++i)
        {
            ...
            {
                if(std::fabs(knobValues[i] - layerEntryKnobValues[i])
                   < kTouchEpsilon)
                {
                    continue;
                }
            ...
            const char* parameterId = activeCore->GetParameterForLayerKnob(
                static_cast<std::size_t>(layer), i);
            if(parameterId[0] == '\0')
            {
                continue;
            }
            ...
            char valueText[24];
            activeCore->FormatParameterValue(
                parameterId, valueText, sizeof(valueText));
    ...
    void UpdateLeds()
    {
        ...
        const auto keyLedValues = bundleMode
                                      ? BundleFieldKeyLedValues()
        ...
        hw.led_driver.SetLed(DaisyField::LED_SW_1,
                             activeLayer == 1 ? 0.85f : 0.05f);
        hw.led_driver.SetLed(DaisyField::LED_SW_2,
                             activeLayer == 2 ? 0.85f : 0.05f);
