...
namespace apps
{
    ...
    void DelayFxAdaptationCore::Prepare(double      sampleRate,
    ...
    PortValue
    DelayFxAdaptationCore::GetPortOutput(const std::string& portId) const
    {
        const auto it = portOutputs_.find(portId);
        return it != portOutputs_.end() ? it->second : PortValue{};
    }

    void DelayFxAdaptationCore::TickUi(double) { BuildDisplay(); }

    bool
    DelayFxAdaptationCore::SetParameterValue(const std::string& parameterId,
                                             float              normalizedValue)
    {
    ...
    DelayFxAdaptationCore::GetControlValue(const std::string& controlId) const
    {
        ...
        const std::string& parameterId) const
    {
        ...
        float value = 0.0f;
        if(!suffix.empty()
           && sharedCore_.GetEffectiveParameterValue(suffix, &value))
        {
            lookup.hasValue = true;
            lookup.value    = value;
        }
    ...
    DelayFxAdaptationCore::CaptureStatefulParameterValues() const
    {
        ...
        if(bundleMode_)
        {
            ...
            snapshots[activeIndex]
                = sharedCore_.CaptureStatefulParameterValues();
        ...
        if(bundleMode_)
        {
            ...
            for(const auto& pair : values)
            {
                ...
                if(IsBundleAlgorithmParameter(suffix))
                {
                    ...
                    requestedIndex = static_cast<std::size_t>(std::round(
                        Clamp01(pair.second)
                ...
                for(std::size_t i = 0; i < descriptors.size(); ++i)
                {
                    const std::string prefix
                        = std::string(descriptors[i].statePrefix) + ".";
                    if(suffix.rfind(prefix, 0) == 0)
                    {
                        algorithmSnapshots_[i].emplace(
                            suffix.substr(prefix.size()), pair.second);
    ...
    DelayFxAdaptationCore::GetParameters() const
    {
        return parameters_;
    }
                                                 ...
                                                 float normalizedValue)
    {
            ...
            if((row == 'a' || row == 'b') && key >= 0 && key < 8)
            {
                    ...
                    else if(key == 5)
                    {
                        if(pressed)
                        {
                            sharedCore_.SetInternalSynthHoldMode(
                                (sharedCore_.GetInternalSynthHoldMode() + 1)
                                % 3);
                        }
                    }
                    else
                ...
                else
                {
                    const std::size_t index
                        = (row == 'a' ? 0 : 8) + static_cast<std::size_t>(key);
        ...
        {
            const auto suffix
                = StripMenuItemPrefix(itemId, LayerSectionName(layer));
            if(!suffix.empty())
            {
                sharedCore_.SetParameterValue(suffix, normalizedValue);
                RefreshSnapshots();
    ...
    std::string
    DelayFxAdaptationCore::MakeParameterId(const std::string& nodeId,
    ...
    std::string
    DelayFxAdaptationCore::MakeMenuRootSectionId(const std::string& nodeId)
    {
        return nodeId + "/menu/root";
    }

    std::string
    DelayFxAdaptationCore::MakeMenuSectionId(const std::string& nodeId,
                                             const std::string& suffix)
    {
    ...
    std::string
    DelayFxAdaptationCore::MakeMenuItemId(const std::string& nodeId,
    ...
    std::string
    DelayFxAdaptationCore::MakeAudioInputPortId(const std::string& nodeId,
    ...
    std::string
    DelayFxAdaptationCore::MakeAudioOutputPortId(const std::string& nodeId,
                                                 std::size_t oneBasedIndex)
    {
    ...
    std::string
    DelayFxAdaptationCore::MakeMidiInputPortId(const std::string& nodeId)
    {
        return nodeId + "/port/midi_in";
    }

    std::string
    DelayFxAdaptationCore::MakeGateInputPortId(const std::string& nodeId)
    {
    ...
    void DelayFxAdaptationCore::BuildMenuModel()
    {
        ...
        for(int layer = 0; layer < 3; ++layer)
        {
            root.items.push_back(
                {MakeMenuItemId(nodeId_, "root", LayerSectionName(layer)),
                 LayerTitle(layer),
                 false,
                 MenuItemActionKind::kEnterSection,
                 0.0f,
        ...
        if(bundleMode_)
        {
            ...
            for(std::size_t i = 0; i < descriptors.size(); ++i)
            {
                algorithms.items.push_back(
                    {MakeMenuItemId(
                         nodeId_, "algorithm", descriptors[i].statePrefix),
                     descriptors[i].label,
                     false,
            ...
            algorithms.items.push_back(
                {MakeMenuItemId(nodeId_, "algorithm", "back"),
                 "Back",
                 false,
        ...
        for(int layer = 0; layer < 3; ++layer)
        {
            ...
            for(std::size_t knob = 0; knob < DaisyDelayFxCore::kKnobCount;
                ++knob)
            {
                ...
                section.items.push_back(
                    {MakeMenuItemId(
                         nodeId_, LayerSectionName(layer), parameterId),
            ...
            section.items.push_back(
                {MakeMenuItemId(nodeId_, LayerSectionName(layer), "back"),
                 "Back",
                 false,
                 MenuItemActionKind::kBack});
    ...
    void DelayFxAdaptationCore::BuildDisplay()
    {
        ...
        display_.texts.push_back(
            {0,
             0,
             bundleMode_ ? CurrentAlgorithmLabel() : GetAppDisplayName(),
             false});
                                                     ...
                                                     bool        saveCurrent)
    {
        ...
        {
            sharedCore_.RestoreStatefulParameterValues(
                algorithmSnapshots_[index]);
        }
        ResetMenuState();
        RefreshSnapshots();
    ...
    {
        return GetDaisyDelayFxAlgorithmDescriptor(sharedCore_.GetSource())
            .label;
    }

    bool
    DelayFxAdaptationCore::SetBundleAlgorithmParameter(float normalizedValue)
    {
        if(!bundleMode_)
        {
            return false;
    ...
    std::string
    DelayFxAdaptationCore::StripControlId(const std::string& controlId) const
    {
    ...
    std::string
    DelayFxAdaptationCore::StripMenuItemPrefix(const std::string& itemId,
