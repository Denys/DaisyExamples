...
- Scope: host-side Daisy Patch plugin and standalone app with a multi-app host layer, `MultiDelay` as the default regression fixture, and `Torus` as the first second app

This checkpoint records the current verified artifact names, the agreed control
map for the `0.2.0` refresh, the Phase 2 multi-app host state, and the latest
...
Rebuild the Patch firmware reference targets when DaisyHost shared cores or
...
successful firmware rebuilds for both `patch/MultiDelay` and `patch/Torus`.
...
- if a knob, `CV 1`, or the menu all target the same parameter, `last touch wins`
...
Input/test-source target behavior for the refresh:

- standalone default = `Host In`
- alternate sources = `Sine`, `Saw`, `Noise`, `Impulse`
...
- `unit_tests` currently passes `36/36`.
...
- The standalone input-mute warning banner is suppressed in the app UI while the underlying host-input mute behavior remains unchanged.
...
- visible standalone icon: not visually rechecked after the final build in this session
- `Host In`, `Sine`, `Saw`, `Noise`, and `Impulse`: build/test covered; manual audio pass not repeated after final UI patch
- app selection and app restore:
  - host/session serialization covered by tests
  - no manual GUI click-through for `multidelay` -> `torus` -> restore was run in this session
- mouse access to every visible control: editor wiring implemented; not exhaustively re-clicked after the final build
...
- encoder menu and side drawer sync: implemented in the refreshed editor; manual recheck still recommended
- all five DSP parameters editable from both menu and live controls: shared core tests pass; final end-to-end manual pass still recommended
- host and Patch firmware menu behavior aligned at the feature level: shared core + firmware adapter build verified
