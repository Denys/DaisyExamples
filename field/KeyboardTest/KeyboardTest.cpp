...
int8_t octaves = 0;
volatile bool trigger_voices = false;

static daisysp::ReverbSc verb;
// Use two side buttons to change octaves.
volatile float kvals[8];
volatile float cvvals[4];
                   ...
                   size_t                                size)
{
    ...
    // Update voice notes when triggered (switch pressed in main loop)
    if(trigger_voices)
    {
