...
template <std::size_t Tracks> class StepSequencer
{
  public:
    ...
    [[nodiscard]] std::uint8_t CurrentStep() const noexcept
    {
    ...
    template <typename Fn> static void ForEachTriggeredTrack(const Tick &tick, Fn &&fn)
    {
        if (!tick.emitted)
            return;
